...
Without it, every contradiction a mistaken ritual link creates is resolved by
retracting a *belief*, transcribing the acquisition error into the belief set.
...
def _wrong_implication() -> PredicateConstraints:
    """Build the constraints for a ritual that coined "every cat is a fish"."""
...
class TestLinkIsTheCulprit:
    ...
    def test_wrong_exclusion_is_found(self) -> None:
        ...
        vocab = PredicateConstraints(exclusion_targets={"cat": {"pet"}})
        assert find_link_culprits(beliefs, [], vocab) == [
            PredicateLink(kind="exclusion", predicate="cat", target="pet"),
        ]

    def test_only_the_guilty_link_is_reported(self) -> None:
        # An innocent second link on the same board must not be blamed.
        vocab = PredicateConstraints(implication_targets={"cat": {"fish"}, "sparrow": {"bird"}})
        assert find_link_culprits(_cat_but_not_fish(), [], vocab) == [
...
class TestLinkIsInnocent:
    def test_rule_driven_contradiction_blames_no_link(self) -> None:
        ...
        vocab = PredicateConstraints(implication_targets={"cat": {"animal"}})
        assert find_link_culprits(beliefs, [guilty_rule], vocab) == []
    ...
    def test_functional_exclusion_is_not_a_candidate(self) -> None:
        ...
        vocab = PredicateConstraints(functional_predicates=("lives_in",))
        assert find_link_culprits(beliefs, [], vocab) == []
...
class TestConstraintReduction:
    def test_without_drops_only_the_named_edge(self) -> None:
        vocab = PredicateConstraints(
            exclusion_targets={"cat": {"dog", "fish"}},
            implication_targets={"cat": {"animal"}},
        )
        reduced = vocab.without(PredicateLink(kind="exclusion", predicate="cat", target="dog"))
