"""Tests for reading a belief's footing off its supports (RFC-0064 §3-2, ADR-0131).

The judgement is placed but not fired (RFC-0064 §6), so nothing in the running
...
it can distinguish (ADR-0112 -- a 0 that never moves is not a result).
...
turn a paged-out antecedent (ADR-0042) into counter-evidence against everything
...
def test_an_empty_record_is_not_a_loss() -> None:
    """``unsupported`` is not ``out`` -- 97.2% of atoms in vivo sit here (ADR-0128)."""
    assert support_verdict([]) == UNSUPPORTED
    assert support_verdict([]) != OUT


def test_the_order_of_the_supports_does_not_matter() -> None:
    """The record keeps derivation order (ADR-0130); the verdict does not read it."""
