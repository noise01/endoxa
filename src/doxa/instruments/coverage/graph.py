...
from doxa.solver import BOOL_SORT, App
...
# (:mod:`doxa.solver`), so it never appears as its own node.
...
def _split_by_polarity(expr: Expr) -> tuple[set[str], set[str]]:
    ...
    ``Implies`` desugars to ``Or(Not(p), q)`` at construction (:mod:`doxa.solver`),
