"""The governance ledger's operation schema (RFC-0063 §2, ADR-0119).

The belief-governance layer (asset ②) has had its machinery since the sixth
period -- TMS revision, preference bands, evidence updates, tie questions -- but
it never had a *shape*. The mechanisms sit scattered across ``kernel/governance/tms``,
``domains/blackboard/evidence.py`` and the event model, so "the governance layer
can be attached to any agent" (RFC-0026 Decision 4) stayed an unverifiable
claim: a boundary that does not exist as a structure cannot be exported.

This module declares that boundary as an **append-only ledger of operations**.
It is a naming exercise, not a new design: every operation below is already
implemented somewhere in the host (RFC-0063 §3's correspondence table), and
:mod:`~doxa.governance.derive` recovers the operation series from
what the host already records.

The seven operations (RFC-0063 §2, granularity fixed by the 2026-07-31 design
decision -- ``confirm``/``refute`` are deliberately *not* folded into one
"record evidence" operation with a polarity argument, so a reader never has to
recover the polarity from an argument):
...
``supersede``
    The world moved on (recency supersession, ADR-0068). The old value was true
    when written -- this is a state change, not a miscalibration.
``confirm`` / ``refute``
    Evidence for / against is booked (ADR-0075). Confidence moves; nothing is
    withdrawn.
``hold``
    The tie no revision preference can separate (ADR-0082). **Both sides stay in
    the view**; this is the first-class form of ``UNRESOLVED``.
``ground``
    An ask-user answer landed. The only operation that confers confidence 1.0
    (ADR-0018), and therefore the only one that directly releases a ``hold``.

**Append-only applies to the schema too** (RFC-0063 §2 decision 4 / §6). A later
increment -- the first-class dependency graph, the Stage 3 temporality DSL --
must be able to fill in what it needs by *adding* operations and columns, never
by changing the meaning of an existing one. The reserved columns below are that
promise made concrete: they are named now and left empty on purpose.

**One of them has since been sat in.** RFC-0064 increment 4 filled
``supported_by`` (ADR-0133), and what that cost is the reservation's own test
result: no column changed meaning, no reader broke, no entry was rewritten -- but
the seat had been declared ``tuple[str, ...]`` and what belongs in it is a set of
*typed* endpoints. Reserving a seat and reserving its dimensions are different
acts, and only the first of them held.

**And one column has since been added that was never reserved.** RFC-0065
increment 1 gave ``confirm``/``refute`` a ``reason`` (ADR-0135). Nothing had
named it in advance, which is the point of the promise being about *adding*
rather than about a fixed list of seats: a column that was never foreseen costs
the same as one that was, so long as no existing column changes meaning.

Pure and basis-independent (stdlib only), like every asset module.
"""
...
#: Why a ``confirm``/``refute`` was booked (RFC-0065 §3-2, ADR-0135). The tuple
...
#: A belief's footing went away and the loss was booked against it (ADR-0132).
REASON_SUPPORT_LOST = "support_lost"
#: A write restated a belief already held, corroborating it instead of
#: overwriting it (ADR-0075 decision 4, ADR-0079).
REASON_REASSERTION = "reassertion"
#: The belief was suspected in a contradiction and revision kept it (ADR-0075).
REASON_REVISION_SURVIVED = "revision_survived"
#: A rule the belief was recorded as resting on was softly retracted (ADR-0080).
...
#: (``domains.blackboard.support``, ADR-0129): ``derivation`` names an atom's node
...
#: by the same host-side test that pins the event names (ADR-0133).
SupportKind = Literal["derivation", "rule"]


@dataclass(frozen=True, slots=True)
class SupportRef:
    """One thing a belief rode on, as the ledger carries it (RFC-0064 increment 4, ADR-0133).
...
class LedgerOp:
    ...
    Attributes:
        ...
        confidence: The confidence the operation itself writes, when it writes
            ...
            the Laplace fold the view replays (RFC-0036).
        ...
        reason: Why this evidence was booked, one of :data:`EVIDENCE_REASONS`
            (RFC-0065 §3-2, ADR-0135). ``None`` for every operation that is not
        ...
        session_id: **Reserved** (RFC-0063 §6, the provenance seat, ADR-0023).
        ...
        supported_by: What the target rested on **at the time of this operation**
            ...
            put there by a derivation (97.2% of atoms in vivo, ADR-0128) and for
            ...
            **the seat's declared type did not**: it was ``tuple[str, ...]``, and
            ...
            turned out to be different acts (ADR-0133).
        valid_at: **Reserved** (RFC-0063 §6, the temporality seat). The Stage 3
            DSL (RFC-0011 §6) carries time as syntax; until then a ledger entry
    ...
    # repurposing one is not (RFC-0063 §6). ``supported_by`` is no longer
    # reserved: RFC-0064 increment 4 sat down in it (ADR-0133).
