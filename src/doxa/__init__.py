"""doxa -- governed beliefs for agents.
...
- ``doxa.syntax`` -- the shape of an atom: predicate, arity, arguments.
- ``doxa.solver`` -- a self-contained SMT engine deciding satisfiability.
- ``doxa.governance`` -- the decision surface, the ledger it writes, and the
  revision machinery that picks what to give up.
- ``doxa.trace`` -- the ordered series of an agent's conscious propositions.
- ``doxa.instruments`` -- calibration and coverage measures, imported by nothing
